...
public class TiedFenceEventHandler {
    ...
    public static void rightClickedFence(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (tag.contains("FirstFence")) {
            firstFence = NbtUtils.readBlockPos(tag.getCompound("FirstFence"));
            if (areFencesUnaligned(firstFence, pos)) {
                tag.remove("FirstFence");
                stack.setTag(tag);
            }
        }
        ...
        if (player != null)
            if (tag.contains("FirstFence")) {
                ...
                tag.put("FirstFence", NbtUtils.writeBlockPos(pos));
                stack.setTag(tag);
                player.getCooldowns().addCooldown(Items.STRING, 5);
            }
    }
